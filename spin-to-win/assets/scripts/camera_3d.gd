...
@export var countdown_animation:AnimatedSprite2D
...
func _ready() -> void:
	rotate_crank = false
	is_launched = false
	countdown_animation.play("default")
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion and rotate_crank:
		...
		if is_launched and not event.is_action_pressed("LMB") and event is InputEventMouseMotion:
			...
			if hit and hit.has("position"):
				self.target_node.global_position = hit["position"]
