...
func _ready() -> void:
	...
	var possible_stats_to_decrease: Array[String] = []
	for stat in possible_stats_to_increase:
		if increase_stat == "Ult":
			if stat == "Ult":
				possible_stats_to_decrease.append(stat)
			elif Global.game_state["stats"][stat] > 1:
				possible_stats_to_decrease.append(stat)
		else:
			if stat == "Ult" or stat == increase_stat:
				continue
			elif Global.game_state["stats"][stat] > 1:
				possible_stats_to_decrease.append(stat)
	
	if possible_stats_to_decrease.is_empty():
		if Global.game_state["stats"]["Ult"] >= 0:
			possible_stats_to_decrease = ["Ult"]
		else:
			possible_stats_to_decrease = [""]
		difficulty_modifier += 1
	
	var decrease_stat: String = possible_stats_to_decrease.pick_random()
	...
	if increase_stat == "Ult" and  Global.game_state["stats"]["Ult"] >= 0:
		decrease_stat = "Ult"
...
func calculate_new_stats() -> Dictionary:
	...
	elif increase_stat != "Ult" and decrease_stat == "Ult":
		new_stats["Ult"] = -1
